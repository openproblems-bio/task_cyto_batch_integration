...
get_ff_annotations <- function(adata, donor_name, layer_name){
  ...
  expression_matrix <- adata$layers[[layer_name]][adata$obs$donor==donor_name,]
  ...
  #return 
  print(dim(ff))
  return_obj <- list('flowframe' = ff, 'ct_annotations' = ct_annotations)
  return(return_obj)
}
...
compute_meta_weights <- function(vector_meta_int, vector_meta_val){
  ...
  meta_prop <- proportions(meta_total)
  return(meta_prop)
}

compute_fs_mapping_similarity <- function(fs_tree_int, ct_ann_int, fs_tree_val, ct_ann_val){
  ...
  meta_int <- factor(paste0("meta ", GetMetaclusters(fs_tree_int)), levels = paste0("meta ",levels(fs_tree_int$metaclustering)))
  meta_val <- factor(paste0("meta ", GetMetaclusters(fs_tree_val)), levels = paste0("meta ",levels(fs_tree_val$metaclustering)))
  ...
  results <- list('similarity' = FSOM_similarity,
