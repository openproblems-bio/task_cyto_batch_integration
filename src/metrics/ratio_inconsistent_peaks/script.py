...
for donor in donor_list:
    ...
    for celltype in celltype_list:
        ...
        # safeguard checking the obsnames are exactly the same
        print(
            "Check that the obsnames are exactly the same between s1 and unintegrated",
            flush=True,
        )
        # Check membership
        split_names = np.concatenate([s1_view_ct.obs_names, s2_view_ct.obs_names])
        u_names = list(u_view_ct.obs_names)

        in_split_not_u = [n for n in split_names if n not in u_names]
        in_u_not_split = [n for n in u_names if n not in split_names]

        if len(in_split_not_u) > 0 or len(in_u_not_split) > 0:
            print(
                f"Error: the obsnames of the unintegrated and s1 and s2 do not match for {donor}, {celltype}."
                "\nPlease check dataset as this should not happen!"
                f"\nMissing (in s1/s2 but not in unintegrated): {in_split_not_u}."
                f"\nExtra (in unintegrated but not in s1/s2): {in_u_not_split}.",
                flush=True,
            )
            sys.exit(1)

        print("Obsnames match!", flush=True)
        ...
        for marker in s1_view_ct.var.index:
            ...
            # persistent_peak_count_u_s1, diagram_u_s1, persistence_values_u_s1 = (
            #     metric_helper.persistent_peak_count(density_dist_u_s1)
            # )
            # persistent_peak_count_u_s2, diagram_u_s2, persistence_values_u_s2 = (
            #     metric_helper.persistent_peak_count(density_dist_u_s2)
            # )
            ...
            # use persistent peak only if the peak calling method is too sensitive...
            # persistent_peak_count_s1, diagram_s1, persistence_values_s1 = (
            #     metric_helper.persistent_peak_count(density_dist_s1)
            # )
            # persistent_peak_count_s2, diagram_s2, persistence_values_s2 = (
            #     metric_helper.persistent_peak_count(density_dist_s2)
            # )
            ...
            print("Comparing peaks between unintegrated and integrated", flush=True)
