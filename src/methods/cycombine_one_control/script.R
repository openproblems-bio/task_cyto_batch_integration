...
# so for our healthy controls, i have to give it the same name, but unique for others..
# as character is needed as otherwise we will get NAs for those healthy controls..
df_to_correct$anchor <- as.character(adata_to_correct$obs$sample)

# get sample name for healthy controls
healthy_controls_sample_name <- as.vector(
    unique(adata_to_correct$obs[adata_to_correct$obs$is_control == 1, "sample"])
)

# change the "anchor" column values for this healthy controls
df_to_correct$anchor[df_to_correct$anchor %in% healthy_controls_sample_name] <- "control"
